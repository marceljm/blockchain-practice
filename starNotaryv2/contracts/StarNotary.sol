...
contract StarNotary is ERC721 {
    ...
    mapping(uint256 => Star) public tokenIdToStarInfo; // token ID => Star
    mapping(uint256 => uint256) public starsForSale; // token ID => price
    ...
    function buyStar(uint256 _tokenId) public payable {
        ...
        if (msg.value > starCost) {
            payable(msg.sender).transfer(msg.value - starCost); // Return the change to the buyer
        }
    }
}
